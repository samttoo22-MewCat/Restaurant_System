...
class reUI():
    def __init__(self, user_id, root):
        ...
        def calWorkingMins():
            ...
            return(working_mins)
        def finish():
            selectedPlace = self.avalibleListbox.curselection()
            text = self.avalibleListbox.get(selectedPlace)
            table_number = int(text[7])

            self.avalibleListbox.delete(selectedPlace)
            self.changeTableState(table_number, "正在使用")
        ...
        self.lbl_3 = tk.Label(self.root, bg = "yellow", text='空桌子桌號', fg='black', font=('微軟正黑體', 15), anchor='center')
        ...
        self.avalibleListbox = tk.Listbox(self.root, font=('微軟正黑體', 11))
        self.avalibleListbox.place(x=80,y=80,width=300,height=300)

        #button set up
        bt_1 = tk.Button(self.root, text='占用', font=('微軟正黑體', 12), command=finish )
        ...
        self.root.after(100, self.updateAvalibleListbox)
        self.root.protocol("WM_DELETE_WINDOW", lambda x = None: self.root.destroy())
    def updateAvalibleListbox(self):
        self.cursor.execute("SELECT table_number FROM r_table where state = '空'")
        newAvalibleListbox = self.cursor.fetchall()
        currentAvalibleListbox = self.avalibleListbox.get(0, tk.END)
        for i in newAvalibleListbox:
            output = "空位 - 第 %d 桌" % i
            if(output not in currentAvalibleListbox):
                self.avalibleListbox.insert(tk.END, "空位 - 第 %d 桌" % i)
        self.root.after(100, self.updateAvalibleListbox)
